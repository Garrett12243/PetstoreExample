...
class UserApi(
   ...
   * @return void
   */
  def updateUser(body: User, username: String) = {
    val await = Try(Await.result(updateUserAsync(body, username), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * @return Future(void)
   */
  def updateUserAsync(body: User, username: String) = {
      helper.updateUser(body, username)
  }
